...
async def get_chat_ids(app: Client):
    ...
    async for dialog in app.get_dialogs():
        if getattr(dialog.chat, 'is_forum', False):
            try:
    ...
    chat_ids = [int(chat_id) for chat_id in chat_ids if str(chat_id).startswith('-')]
...
async def send_last_message_to_groups(apps, timee, numtime):
    ...
    tasks = [send_last_message(app) for app in apps]
